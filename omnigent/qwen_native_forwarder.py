...
async def forward_qwen_events_to_session(
    *,
    ...
    # Latched death. The probe is throttled to _TMUX_PROBE_INTERVAL_S while the
    # poll runs every poll_interval_s, so a verdict has to SURVIVE the polls
    # that did not re-probe -- otherwise the drain and the terminal edge would
    # flicker back off between probes. Fresh stream bytes clear it.
    pane_dead = False
        ...
        while True:
            try:
                ...
                if state.turn_open and now - last_probe_at >= _TMUX_PROBE_INTERVAL_S:
                    ...
                    if verdict is None:
                        # Inconclusive: start the grace clock, or leave a
                        # running one alone. Only a CONCLUSIVE verdict (or new
                        # bytes) may clear it -- a throttled poll must not, or
                        # the clock restarts ~12 times between probes and the
                        # grace can never expire, which is precisely the
                        # "unreachable tmux strands the parent forever" case
                        # this window exists to bound.
                        if unknown_since is None:
                            unknown_since = now
                    else:
                        unknown_since = None
                        pane_dead = not verdict
                # An inconclusive verdict STANDS between probes, so the grace is
                # evaluated on every poll rather than only on probing ones.
                if unknown_since is not None and not pane_dead:
                    if now - unknown_since >= _LIVENESS_UNKNOWN_GRACE_S:
                        _logger.warning(
